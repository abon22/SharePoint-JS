...
(function ($, undefined)
{
    $("document").ready(function ()
    {
        ...
        ExecuteOrDelayUntilScriptLoaded(Get_Infraction_ItemID, "sp.js");
...
function Get_EmployeeProfile_ItemID(){
    ...
    old_department=getCookie("old_department");
        console.log("old_department: "+old_department);
    ...
    caml.set_viewXml("<view><Query><Where><Eq><FieldRef Name='LI_Department' /><Value Type='Text'>"+old_department+"</Value></Eq></Where></Query></view>");
    ...
    // }
     ...
     if(old_department!=''){
       ...
       if(EmployeeProfile_ItemID.length >= 1){
        for (var i = 0; i<=EmployeeProfile_ItemID.length-1; i++){
          Update_EmployeeProfile_Departments(EmployeeProfile_ItemID[i]);
          // Update_Infraction_Departments(EmployeeProfile_ItemID[i]);
          //console.log("PRESAVE :"+ EmployeeProfile_ItemID[i]);

        } 

        setCookie("old_department", "", 30);     
      }
    }
...
function Update_EmployeeProfile_Departments(ID){
    ...
    new_department=getCookie("new_department");
    ...
    oListItem.set_item('LI_Department', new_department);
    oListItem.update();
...
var returnedItems_Infraction;

var Infraction_ItemID = [];

function Get_Infraction_ItemID(){
    ...
    var context = new SP.ClientContext();
    var list = context.get_web().get_lists().getByTitle('Infraction Point System');
    var caml = new SP.CamlQuery();
    caml.set_viewXml("<view><Query><Where><Eq><FieldRef Name='LI_EmployeeDepartment' /><Value Type='Text'>"+old_department+"</Value></Eq></Where></Query></view>");
    returnedItems_Infraction = list.getItems(caml);
    context.load(returnedItems_Infraction);
    context.executeQueryAsync(Get_Infraction_ItemID_Succeeded, Query_Failed);

}

function Get_Infraction_ItemID_Succeeded(){
    var enumerator = returnedItems_Infraction.getEnumerator();

    while (enumerator.moveNext()) {
        var listItem = enumerator.get_current();  
        Infraction_ItemID.push(listItem.get_item('ID'));
    }

     if(old_department!=''){

       if(Infraction_ItemID.length >= 1){
        for (var i = 0; i<=Infraction_ItemID.length-1; i++){
          Update_Infraction_Departments(Infraction_ItemID[i]);
          //console.log("PRESAVE :"+ EmployeeProfile_ItemID[i]);
...
function Update_Infraction_Departments(ID){
    //final_department = document.getElementById("Title_fa564e0f-0c70-4ab9-b863-0177e6ddd247_$TextField").value;
    ...
    oListItem = oList.getItemById(ID);
    oListItem.set_item('LI_EmployeeDepartment', new_department);
    oListItem.update();
    clientContext.executeQueryAsync();
