...
function Get_PurchaseRequest_TO_Edit_Suceeded() {
  ...
  while (listItemsCollection.moveNext()) {
    ...
    $(".ui.selection.dropdown.col.s12.INPUT_PROJECT_TYPE").dropdown(
      "set selected",
      listItem.get_item("Project_x0020_Type")
    );
    ...
    $(
      ".ui.selection.dropdown.item_status.col.s12.DIV_EXECUTIVE_APPROVAL"
    ).dropdown("set selected", listItem.get_item("Approval"));
    $(".ui.selection.dropdown.item_status.col.s12.DIV_EXEPENSE_TYPE").dropdown(
      "set selected",
      listItem.get_item("Expense_x0020_Type")
    );
    $(".ui.selection.dropdown.item_status.col.s12.DIV_SET_APPROVER").dropdown(
      "set selected",
      listItem.get_item("Set_x0020_Approver")
    );
    $(".ui.selection.dropdown.item_status.col.s12.DIV_VP_APPROVAL").dropdown(
      "set selected",
      listItem.get_item("VP_x0020_Approval")
    );
    $(".ui.selection.dropdown.item_status.col.s12.DIV_CEO_APPROVAL").dropdown(
      "set selected",
      listItem.get_item("CEO_x0020_Approval")
    );
    $(
      ".ui.selection.dropdown.item_status.col.s12.DIV_INITIAL_APPROVER"
    ).dropdown("set selected", listItem.get_item("Initial_x0020_Approver"));
...
function onRequestSuccess() {
  ...
  if (
    ...
    requestorName != "Gwen Stephanie Yurag" &&
    requestorName != "Lito Abon Jr." &&
  ...
  if (Approval_Stage == "Initial Approval") {
    ...
    if (
      ...
      requestorName != "Artem Soukhomlinov" &&
      requestorName != "Artem Soukhomlinov | OffshoringTBOS.com" &&
      requestorName != "Mithi Juacel Sandalo" &&
      requestorName != "Ross Handley"
    ) {
      if (requestorName != current_user) {
        document.getElementById("TR_EXECUTIVE_APPROVAL").style.display = "none";
        document.getElementById("DIV_INITIAL_APPROVER_DROPDOWN").className +=
          " disabled";
      } else {
        document.getElementById("TR_EXECUTIVE_APPROVAL").style.display = "none";
      }
  ...
  } else if (Approval_Stage == "Finance Approval") {
    document.getElementById("DIV_INITIAL_APPROVER_DROPDOWN").className +=
      " disabled";
    ...
    document.getElementById("TR_REQUEST_STATUS").style.display = "none";
    if (
      requestorName != "Cherry Yurag" &&
      requestorName != "Jestine Ann Lopezbaños"
    ) {
  ...
  } else if (Approval_Stage == "Finance Closed") {
    console.log("Approval Stage : " + Approval_Stage);
    document.getElementById("DIV_INITIAL_APPROVER_DROPDOWN").className +=
      " disabled";
    document.getElementById("DIV_APPROVAL").style.display = "none";

    //SHOW FINANCE CLOSED TAG
    ...
    //SHOW REQUESTOR CLOSED TAG
    document.getElementById("DIV_REQUESTOR_CLOSED").style.display = "none";
    document.getElementById("SPAN_REQUESTOR_CLOSED").style.display = "block";
  }
}
//-----------------------------------------------------------------------------------------------------------------
// VALIDATE ADD ITEM
